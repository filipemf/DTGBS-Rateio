...
function readExcelFileFlat(sheetName) {
  ...
  if (sheet) {
    ...
    Swal.fire({
      ...
      if (result.value && /^[0-9,]+$/.test(result.value) && !isNaN(Date.parse(document.getElementById('startDate').value)) && !isNaN(Date.parse(document.getElementById('endDate').value))) {
        ...
        // Divide result.value.replace(/,/g, '.') by the total count for each Cód Ccusto
        const dividedResults = {};
        for (const codCcusto in codCcustoCounts) {
          if (codCcustoCounts.hasOwnProperty(codCcusto)) {
            dividedResults[codCcusto] = result.value.replace(/,/g, '.') / Object.keys(codCcustoCounts).length;
          }
        }
        ...
        const tableHtml = `
          <div style='display: inline-grid; margin: 30px; min-width: 500px'>
            ...
            <h4 style='font-weight: 900'>Valor do Rateio: ${result.value.replace('.', /,/g)}</h4>
  ...
  if (sheet) {
    ...
    Swal.fire({
      ...
      if (result.value && /^[0-9,]+$/.test(result.value) && !isNaN(Date.parse(document.getElementById('startDate').value)) && !isNaN(Date.parse(document.getElementById('endDate').value))) {
        ...
        for (const codCcusto in codCcustoCounts) {
          if (codCcustoCounts.hasOwnProperty(codCcusto)) {
            dividedResults[codCcusto] = result.value.replace(/,/g, '.') / totalCount * codCcustoCounts[codCcusto];
          }
        }
...
function readExcelFileByValue(sheetName) {
  const file = dialog.showOpenDialogSync({
    properties: ['openFile'],
    filters: [
      { name: 'Excel Files', extensions: ['xlsx', 'xls'] },
  ...
  if (sheet) {
    ...
    Swal.fire({
      ...
      if (result.value && /^[0-9,]+$/.test(result.value) && !isNaN(Date.parse(document.getElementById('startDate').value)) && !isNaN(Date.parse(document.getElementById('endDate').value))) {
        ...
        for (const codCcusto in codCcustoCounts) {
          if (codCcustoCounts.hasOwnProperty(codCcusto)) {
            dividedResults[codCcusto] = result.value.replace(/,/g, '.') * codCcustoCounts[codCcusto];
          }
        }
        ...
        const tableHtml = `
          <div style='display: inline-grid; margin: 30px; min-width: 500px'>
            ...
            <h4 style='font-weight: 900'>Tipo do Rateio: Por Valor</h4>
            <h4 style='font-weight: 900'>Valor do Rateio: ${result.value.replace('.', /,/g)}</h4>
            <h4 style='font-weight: 900'>Data de ínicio: ${formattedStartDate}</h4>
            <h4 style='font-weight: 900'>Data final: ${formattedEndDate}</h4>
            <h4 style='font-weight: 900'>Head Count Total: ${sum}</h4>
